			...
			try {
				if(optParams.listener !== undefined && optParams.callback !== undefined) {
					...
					xmlhttp.open("GET", Dynamix.base_url + ":"+Dynamix.port
						+ "/opendynamixsession?sessionListenerId="+listenerId+
					...
					Dynamix.Callbacks[callbackId] = optParams.callback;
					xmlhttp.open("GET", Dynamix.base_url + ":"+Dynamix.port
						+ "/opendynamixsession?callbackId="+callbackId);
				} else {
					xmlhttp.open("GET", Dynamix.base_url + ":"+Dynamix.port
						+ "/opendynamixsession");
				}
...
**/
			this.addContextSupport = function(contextType, pluginId, optParams) {
				try {
					var xmlhttp = Dynamix.getXmlHttpRequest();
...
dynamixContextHandler.contextRequest("org.ambientdynamix.contextplugins.speechtotext.voiceresults", 
	"org.ambientdynamix.contextplugins.speechtotext", voiceControlContextRequestCallback );
**/
			this.contextRequest = function(contextType, pluginId, callback) {
				try {
					var xmlhttp = Dynamix.getXmlHttpRequest();
					var callbackId = Dynamix.generateGuid();
					Dynamix.Callbacks[callbackId] = callback;
...
@private
*/
			makeConfiguredRequest = function(url, method, contextType, pluginId, optParams) {
...
dynamixContextHandler.addConfiguredContextSupport("PUT", "context.type.1", "plugin.id.1", {callback : configuredRequestCallback, params : params, headers : headerObject});
**/
			this.configuredContextRequest = function(method, contextType, pluginId, optParams) {
				makeConfiguredRequest("/configuredcontextrequest", method, contextType, pluginId, optParams);
			};
...
var paramsObject = {color : "red", lux : 22};
var headerObject = {"Content-type" : "application/x-www-form-urlencoded"};
dynamixContextHandler.addConfiguredContextSupport("PUT", "context.type.1", "plugin.id.1", {callback : configuredRequestCallback, listener : configuredRequestListener, params : paramsObject, headers : headerObject});
**/
			this.addConfiguredContextSupport = function(method, contextType, pluginId, optParams) {
				makeConfiguredRequest("/addconfiguredcontextsupport", method, contextType, pluginId, optParams);
			};
/**
			...
			this.removeContextSupportForContextType = function(contextType, optParams) {
				try {
					...
					if(optParams.callback!==undefined){
						...
						Dynamix.Callbacks[callbackId] = optParams.callback;
						xmlhttp.open("GET", Dynamix.base_url + ":" + Dynamix.port + 
							"/removecontextsupportforcontexttype?contextHandlerId=" + this.id 
							+ "&contextType=" + contextType + "&callbackId="+callbackId);
					} else {
						xmlhttp.open("GET", Dynamix.base_url + ":" + Dynamix.port + 
							"/removecontextsupportforcontexttype?contextHandlerId=" + this.id
			...
			this.removeContextSupportForSupportId = function(supportId, optParams) {
				try {
					...
					if(optParams.callback!==undefined){
						...
						Dynamix.Callbacks[callbackId] = optParams.callback;
						xmlhttp.open("GET", Dynamix.base_url + ":" + Dynamix.port + 
							"/removecontextsupportforsupportid?contextHandlerId=" + this.id 
							+ "&supportId=" + supportId + "&callbackId="+callbackId);
					} else {
						xmlhttp.open("GET", Dynamix.base_url + ":" + Dynamix.port + 
							"/removecontextsupportforcontexttype?contextHandlerId=" + this.id
							+ "&supportId=" + supportId);
		...
		bindHelper : function(index) {
			...
			if (Dynamix.binding) {
				...
				if(Dynamix.pairing_token==null){
					...
					xmlhttp.open("GET", Dynamix.base_url + ":" + Dynamix.port
					+ "/dynamixBind?pairing_token="+Dynamix.pairing_token, true);
				}
